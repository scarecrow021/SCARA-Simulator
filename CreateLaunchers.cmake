...
message("Warning OL! CMP0026 set to OLD !")
cmake_policy(SET CMP0026 OLD)

include(CleanDirectoryList.cmake)
...
macro(_launcher_produce_vcproj_user)
	if(MSVC)
    # Beautiful trick to allow multiple configuration definitions
    set(TEMP "")
    set(DUMP_AFTER 0)
    if(EXISTS ${VCPROJNAME}.${VCPROJ_TYPE}.${USERFILE_EXTENSION})
      file(STRINGS ${VCPROJNAME}.${VCPROJ_TYPE}.${USERFILE_EXTENSION} TEMP NEWLINE_CONSUME)      
      string(FIND ${TEMP} "<PropertyGroup" COUNT)
      string(SUBSTRING ${TEMP} "${COUNT}-1" "-1" TEMP)

      string(FIND "${TEMP}" "</Project>" COUNT)
      string(SUBSTRING "${TEMP}" "0" ${COUNT} TEMP)
      set(DUMP_AFTER 1)
    endif()
		file(READ
      ...
      if(NOT ${FLAVOR} STREQUAL "")
        STRING(COMPARE EQUAL ${USERFILE_CONFIGNAME} ${FLAVOR} TEST)
      endif()
      if(${TEST})
        file(TO_NATIVE_PATH
    ...
    if(${DUMP_AFTER})
      set(USERFILE_CONFIGSECTIONS ${TEMP}${USERFILE_CONFIGSECTIONS})
    endif()
...
function(clear_launcher _targetname)
  set(VCPROJNAME "${CMAKE_CURRENT_BINARY_DIR}/${_targetname}")
  _launcher_system_settings()
  if(EXISTS ${VCPROJNAME}.${VCPROJ_TYPE}.${USERFILE_EXTENSION})
    file(REMOVE ${VCPROJNAME}.${VCPROJ_TYPE}.${USERFILE_EXTENSION})
    message("Removed ${VCPROJNAME}.${VCPROJ_TYPE}.${USERFILE_EXTENSION}")
  else()
    message("COULD NOT FIND ${VCPROJNAME}.${VCPROJ_TYPE}.${USERFILE_EXTENSION}")
  endif()
endfunction()
...
function(create_target_launcher_special _targetname _projectname)
	_launcher_system_settings()
	_launcher_process_args(${ARGN})

	set(VCPROJNAME "${CMAKE_CURRENT_BINARY_DIR}/${_projectname}")
	_launcher_produce_vcproj_user()
